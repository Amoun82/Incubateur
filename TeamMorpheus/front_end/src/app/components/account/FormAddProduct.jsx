...
const vadilSchema = Yup.object().shape({
    ...
    description: Yup.string()
        ...
        .max(1000, 'Too Long!')
        .required('Required'),
    material: Yup.string().min(3, 'Too Short!').max(20, 'Too Long!').required('Required'),
    brand: Yup.string().min(5, 'Too Short!').max(20, 'Too Long!').required('Required'),
    quantity: Yup.string()
        .min(1, 'Min value 1')
        .max(3, 'Max value 999')
        .required('Required')
        .matches(/^(?!0*(\.0+)?$)(\d+|\d*\.\d+)$/, 'cannot be negative and not null'),
    price: Yup.string()
        .min(1, 'Min value 1')
        .max(8, 'Max value 999')
        .required('Required')
        .matches(/^[0-9].+$/, 'cannot be negative'),
    imglink: Yup.string()
        .min(10, 'Too Short!')
        .max(200, 'Too Long!')
        .required('Required'),
...
export default class FormAddProduct extends React.Component {
    ...
    getColor() {
        axios
            .get(`https://127.0.0.1:8000/api/color`)
            .then((res) => {
                //console.log(typeof res.data);
                this.setState({
                    color: res.data,
                        ...
                        axios
                            ...
                            .then((res) => {
                                ...
                                if (res.data[0] === '200') {
                                    ...
                                    window.open(
                                        `http://localhost:3000/product/productid=${res.data[1]}`,
                                        '_self',
                                    );
                                }
                            })
                                    ...
                                    <div className="grid grid-cols-2 lg:gap-y-10 lg:gap-x-40">
                                        ...
                                        <div className="grid grid-rows-2">
                                            ...
                                            <Field
                                                ...
                                                className="bg-transparent border-t-0 border-l-0 border-r-0 border-b-4 border-white placeholder-white font-roboto text-[#00cbd3]"
                                            >
                                                {Object.entries(this.state.category).map(
                                                    ([key, value]) => {
                                                        return (
                                                            <option
                                                                key={key}
                                        ...
                                        <div className="grid grid-rows-2">
                                            ...
                                            <Field
                                                ...
                                                className="bg-transparent border-t-0 border-l-0 border-r-0 border-b-4 border-white placeholder-white font-roboto text-[#00cbd3]"
                                            >
                                                {Object.entries(this.state.color).map(
                                                    ([key, value]) => {
                                                        return (
                                                            <option
                                                                key={key}
                                        ...
                                        <div className="grid grid-rows-2">
                                            ...
                                            <Field
                                                ...
                                                className="bg-transparent border-t-0 border-l-0 border-r-0 border-b-4 border-white placeholder-white font-roboto text-[#00cbd3]"
                                            >
                                                {Object.entries(this.state.weight).map(
                                                    ([key, value]) => {
                                                        return (
                                                            <option
                                                                key={key}
